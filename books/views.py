...
from books.forms import BookrankForm
...
def book_detail(request, pk):
    ...
    if request.method=='GET' :
        bookrankavg = Bookrank.objects.filter(book=pk).aggregate(Avg('rank'))['rank__avg']
        context = {
            'book': book,
            'form': form,
            'bookrankavg': bookrankavg
        }
        return render(request, 'book_detail.html', context)
    elif request.method == 'POST':
        form = BookrankForm(request.POST)
        if form.is_valid() and request.user.is_authenticated:
            ...
            books_recommended(request.user)
            bookrank.save()
        bookrankavg = Bookrank.objects.filter(book=pk).aggregate(Avg('rank'))['rank__avg']
        context = {
            'book': book,
...
def books_recommended(user):
    booksrank = Bookrank.objects.filter(user=user)
    for bookrank in booksrank:
        if bookrank.rank > 3 :
            book=Book.objects.get(pk=bookrank.book.id)
            print(book.title)